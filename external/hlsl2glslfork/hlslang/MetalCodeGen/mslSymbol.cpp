...
static bool IsReservedMslKeyword (const std::string& name)
{
...
static bool IsMslBuiltin (const std::string& name)
{
	static const char* s_reservedKeywords[] = {
		"radians",
		"degrees",
...
MslSymbol::MslSymbol( const std::string &n, const std::string &s, const std::string &r, int id, EMslSymbolType t, TPrecision prec, EMslQualifier q, int as )
 : MslSymbolOrStructMemberBase(n, s, t, q, prec, as),
   registerSpec(r),
   identifier(id),
   mangleCounter(0),
   structPtr(0),
   ...
   refCount(0)
{
	if (IsReservedMslKeyword(n) || IsMslBuiltin(n))
	{
		name = "xlat_var" + n;
	}
	mangledName = name;
...
void MslSymbol::writeDecl (std::stringstream& out, WriteDeclMode mode)
{
    /*
	switch (qual)
	{
